...
class SliceDataset(torch.utils.data.Dataset):
    ...
    def _retrieve_metadata(self, fname):
        with h5py.File(fname, "r") as hf:
            ...
            # Does not print - problem is not this far
            # printing - removed for training :) 06/07/22
    ...
    def __getitem__(self, i: int):
        ...
        with h5py.File(fname, "r") as hf:
            ...
            image = hf["motion"][:, :, dataslice]

            # now slicing correct index!
            target = hf["still"][:, :, dataslice] 
            # print(f'FILE: motion_mri_data.py\n  target: {target}')
            # print(f"FILE: motion_mri_data.py\n  target {target.shape}, dtype {target.dtype}")
            # target (64, 64), dtype float64
        ...
        if self.transform is None:
            sample = (image, target, attrs, fname.name, dataslice)
        else:
            sample = self.transform(image, target, attrs, fname.name, dataslice)
